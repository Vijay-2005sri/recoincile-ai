...
THEME_TOKENS: dict[str, dict[str, str]] = {
    ...
    "Light": {
        ...
        "text-primary": "#101828", "text-secondary": "#344054", "text-muted": "#475467", "text-inverse": "#FFFFFF", "text-on-action": "#FFFFFF",
...
def inject_theme(motion_enabled: bool = True, theme_mode: str = "Dark") -> None:
    ...
    css = dedent(
        ...
        html,body,[class*="css"],.stApp { font-family:"Palatino Linotype",Palatino,Georgia,serif; }
        [data-testid="stSidebar"],[data-testid="stSidebar"] *, .rail-title,.rail-sub { font-family:"Inter",-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif!important; }
        [data-testid="stSidebar"] [data-testid="stSidebarCollapseButton"],[data-testid="stSidebar"] [data-testid="stSidebarCollapseButton"] *,[data-testid="stExpandSidebarButton"],[data-testid="stExpandSidebarButton"] *,[data-testid="stSidebar"] .material-symbols-rounded,[data-testid="stSidebar"] .material-symbols-outlined { font-family:"Material Symbols Rounded","Material Symbols Outlined",sans-serif!important; font-style:normal!important; font-weight:normal!important; }
        /* A resilient visible fallback: never expose Streamlit's internal
           material-icon name if its webfont has not loaded yet. */
        [data-testid="stSidebar"] [data-testid="stSidebarCollapseButton"] * { font-size:0!important; }
        [data-testid="stSidebar"] [data-testid="stSidebarCollapseButton"] button::after { content:"«"; display:block; color:var(--text-secondary); font-family:"Inter",sans-serif; font-size:1.45rem; font-weight:400; line-height:1; }
        [data-testid="stDataFrame"],[data-testid="stDataFrame"] *,[data-testid="stTable"],[data-testid="stTable"] * { font-family:"Palatino Linotype",Palatino,Georgia,serif!important; }
        .stApp h1,.stApp h2,.stApp h3,.stApp h4,.stApp h5,.stApp h6,.page-title,.page-kicker,.panel-title,.command-bar,.metric-value { font-family:"Inter",-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif!important; }
        .metric-value,[data-testid="stMetricValue"],[data-testid="stMetricValue"] *,[data-testid="stDataFrame"] [role="gridcell"][data-column-type="number"] { font-family:"Inter",-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif!important; font-variant-numeric:tabular-nums; }
        .stApp h1,.stApp h2,.stApp h3,.stApp h4,.stApp h5,.stApp h6,[data-testid="stMarkdownContainer"],[data-testid="stMarkdownContainer"] p,[data-testid="stWidgetLabel"] p,[data-testid="stWidgetLabel"] span { color:var(--text-primary)!important; opacity:1!important; } [data-testid="stCaptionContainer"] { color:var(--text-muted)!important; opacity:1!important; }
        ...
        #MainMenu,[data-testid="stDecoration"],footer { display:none!important; } [data-testid="stHeader"] { height:3.5rem; background:transparent; pointer-events:none; } [data-testid="stToolbar"] { display:flex!important; pointer-events:none; } [data-testid="stExpandSidebarButton"] { position:fixed!important; z-index:1000!important; top:.72rem; left:.72rem; width:2.35rem!important; height:2.35rem!important; min-height:2.35rem!important; padding:0!important; border:1px solid var(--border-strong)!important; border-radius:.7rem!important; background:var(--surface-1)!important; color:var(--text-primary)!important; box-shadow:0 .5rem 1.3rem color-mix(in srgb,var(--shadow-color) 18%,transparent)!important; pointer-events:auto!important; } [data-testid="stExpandSidebarButton"]:hover { border-color:var(--accent-info)!important; background:var(--surface-2)!important; }

        [data-testid="stSidebar"],[data-testid="stSidebar"]>div:first-child { background:var(--bg-sidebar)!important; border-right:1px solid var(--border-subtle); box-shadow:1rem 0 2.8rem color-mix(in srgb,var(--shadow-color) 22%,transparent); }
        /* This is a resizable flex panel. Width alone does not release its
           layout space when collapsed; its flex basis must also become zero. */
        [data-testid="stSidebar"][aria-expanded="false"] { flex:0 0 0!important; width:0!important; min-width:0!important; max-width:0!important; }
        [data-testid="stAppViewContainer"]:has([data-testid="stSidebar"][aria-expanded="false"]) [data-testid="stMain"] { width:100%!important; }
        [data-testid="stAppViewContainer"]:has([data-testid="stSidebar"][aria-expanded="false"]) .command-bar { left:4.2rem!important; }
        ...
        [data-testid*="stBaseButton-segmented_control"] { color:var(--text-primary)!important; background:var(--surface-2)!important; border-color:var(--border-subtle)!important; opacity:1!important; } [data-testid*="stBaseButton-segmented_control"] *,[data-testid*="stBaseButton-segmented_control"] button { color:var(--text-primary)!important; background:var(--surface-2)!important; border-color:var(--border-subtle)!important; opacity:1!important; } [data-testid*="stBaseButton-segmented_controlActive"],[data-testid*="stBaseButton-segmented_controlActive"] * { color:var(--text-primary)!important; background:var(--accent-info-soft)!important; border-color:var(--accent-info)!important; }
        .rail-brand { display:flex; align-items:center; gap:.7rem; padding:.55rem .2rem 1.05rem; margin-bottom:.8rem; border-bottom:1px solid var(--border-subtle); }.rail-logo { display:grid; place-items:center; width:2.15rem; height:2.15rem; border-radius:.65rem; background:var(--accent-action); color:var(--text-on-action); box-shadow:0 .55rem 1.5rem color-mix(in srgb,var(--accent-action) 30%,transparent); font-weight:820; }.rail-title { color:var(--text-primary); font-weight:760; letter-spacing:-.02em; }.rail-sub { color:var(--text-muted); font-size:.67rem; }.rail-foot { margin-top:1rem; padding:.8rem; border:1px solid var(--border-subtle); border-radius:.75rem; background:var(--surface-2); color:var(--text-secondary); font-size:.72rem; line-height:1.45; }.st-key-theme_switch_panel { position:relative; display:inline-block; width:1.8rem; margin:.25rem 0 .7rem; }.st-key-theme_switch_panel .stButton>button { width:1.8rem!important; height:1.8rem!important; min-height:1.8rem!important; padding:0!important; border-radius:50%!important; border-color:var(--accent-info)!important; background:var(--accent-info-soft)!important; color:var(--accent-info)!important; font-size:.76rem!important; line-height:1!important; box-shadow:0 0 .72rem color-mix(in srgb,var(--accent-info) 34%,transparent)!important; }.st-key-theme_switch_panel .stButton>button:hover { background:var(--surface-3)!important; border-color:var(--focus)!important; transform:translateY(-1px); }.theme-mode-flash { position:absolute; z-index:8; top:.37rem; left:2.2rem; padding:.28rem .46rem; border:1px solid var(--border-strong); border-radius:.45rem; background:var(--surface-1); color:var(--text-primary); font-size:.64rem; font-weight:700; line-height:1; white-space:nowrap; pointer-events:none; box-shadow:0 .45rem 1rem color-mix(in srgb,var(--shadow-color) 22%,transparent); animation:theme-mode-flash .1s ease-out forwards; } @keyframes theme-mode-flash { from { opacity:0; transform:translateY(.22rem); } 35% { opacity:1; transform:translateY(0); } to { opacity:0; transform:translateY(-.18rem); } }
        ...
        .feed-status { display:grid; grid-template-columns:minmax(12rem,.9fr) minmax(0,2.1fr); gap:1rem; align-items:center; margin:0 0 1rem; padding:.82rem .95rem; border:1px solid var(--border-subtle); border-radius:var(--radius); background:var(--surface-1); box-shadow:0 .65rem 1.7rem color-mix(in srgb,var(--shadow-color) 10%,transparent); }.feed-kicker { color:var(--accent-info); font-size:.64rem; font-weight:760; letter-spacing:.07em; text-transform:uppercase; }.feed-title { margin-top:.16rem; color:var(--text-primary); font-size:.92rem; font-weight:760; }.feed-copy { margin-top:.18rem; color:var(--text-secondary); font-size:.7rem; line-height:1.4; }.feed-meta { display:flex; flex-wrap:wrap; justify-content:flex-end; gap:.5rem; }.feed-source { min-width:6rem; padding:.43rem .54rem; border:1px solid var(--border-subtle); border-radius:.56rem; background:var(--surface-2); }.feed-source strong { display:block; color:var(--text-primary); font-size:.69rem; }.feed-source span { display:block; margin-top:.08rem; color:var(--text-muted); font-size:.62rem; }.feed-source.missing { border-color:var(--status-critical); background:var(--status-critical-soft); }.feed-state { display:inline-flex; align-items:center; gap:.3rem; margin-top:.36rem; padding:.24rem .42rem; border:1px solid var(--tone); border-radius:999px; background:color-mix(in srgb,var(--tone) 10%,var(--surface-1)); color:var(--tone); font-size:.6rem; font-weight:760; }.feed-state::before { content:""; width:.42rem; height:.42rem; border-radius:50%; background:var(--tone); box-shadow:0 0 .55rem var(--tone); }.feed-state.ready { --tone:var(--status-success); }.feed-state.waiting { --tone:var(--status-warning); }.st-key-command_feed_action .stButton>button { width:100%; margin-bottom:1rem; }
        ...
        [data-testid="stDataFrame"],[data-testid="stPlotlyChart"] { overflow:hidden; border:1px solid var(--border-subtle); border-radius:.8rem; background:var(--surface-1); box-shadow:0 .75rem 1.8rem color-mix(in srgb,var(--shadow-color) 10%,transparent); }.stDataFrame *,[data-testid="stDataFrame"] * { color:var(--text-primary)!important; } [data-testid="stExpander"] { overflow:hidden; border:1px solid var(--border-subtle); border-radius:.7rem; background:var(--surface-1); } [data-testid="stExpander"] summary,[data-testid="stExpander"] summary:hover,[data-testid="stExpander"] summary:focus-visible,[data-testid="stExpander"] [role="button"] { background:var(--surface-1)!important; color:var(--text-primary)!important; opacity:1!important; } [data-testid="stExpander"] summary:hover { background:var(--surface-2)!important; } [data-testid="stExpander"] summary *,[data-testid="stExpander"] summary svg,[data-testid="stExpander"] summary svg * { color:var(--text-primary)!important; fill:currentColor!important; stroke:currentColor!important; opacity:1!important; } [data-testid="stAlert"] { border-radius:.7rem; color:var(--text-primary); }
...
def data_feed_status(data_mode: str, frames: dict, source_ready: bool, last_processed: str | None) -> None:
    """Summarise the staged input feed without duplicating intake controls."""
    source_type = "Demonstration feed" if data_mode == "demo" else "Uploaded data feed"
    description = "Bundled synthetic files are staged for a safe demonstration." if data_mode == "demo" else "Custom files are staged locally and ready for validation."
    status = "Ready to reconcile" if source_ready else "Sources required"
    status_class = "ready" if source_ready else "waiting"
    cards = []
    for name in ("orders", "payments", "settlements"):
        frame = frames.get(name)
        state = "staged" if frame is not None else "missing"
        count = f"{len(frame):,} rows" if frame is not None else "not provided"
        cards.append(f'<div class="feed-source {state}"><strong>{escape(name.title())}</strong><span>{escape(count)} · {state}</span></div>')
    last_run = f" Last batch: {escape(last_processed)}." if last_processed else " No batch has run yet."
    st.markdown(
        f'<section class="feed-status"><div><div class="feed-kicker">Data feed</div><div class="feed-title">{escape(source_type)}</div><div class="feed-copy">{escape(description)}{last_run}</div><span class="feed-state {status_class}">{escape(status)}</span></div><div class="feed-meta">{"".join(cards)}</div></section>',
...
def reconciliation_core(counts: dict[str, int], flow_active: bool = False) -> None:
    """Render the real batch path, replaying only after a reconciliation run."""
    payload = {name: max(0, int(counts.get(name, 0))) for name in ("orders", "payments", "settlements", "verified", "exceptions")}
    reconciliation_scene(payload, theme_tokens(), flow_active)
...
def exception_radar(results, affected_amounts: dict[str, float]) -> None:
    ...
    risk_badge = f"{format_breakdown(affected_amounts, compact=True)} AT RISK"
...
def decision_card(result: dict) -> None:
    ...
    def money(value) -> str:
        return format_money(value, result.get("currency", "INR"))
